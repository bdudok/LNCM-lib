...
class Ripples(object):
    def __init__(self, procpath, prefix, fs=2000, bands=('theta', 'ripple'), config=None, enum=False, force=False,
        ...
        self.channels = None
        self.ephys_channel = ephys_channel
        if prefix is not None:
            self.path = os.path.join(procpath, prefix, '_ripples//')
            self.read_phys()
    ...
    # This was used in legacy. now, ephys is passed (from ImagingSession) as Ephys object.
    def read_phys(self):
        ch = self.ephys_channel
        self.ephys = Ephys(self.procpath, self.prefix, channel=ch)
        self.trace = self.ephys.trace
        self.frame_array = self.ephys.frames
        self.n = len(self.trace)
        self.ephys_all_channels = self.ephys.edat[1:]
    ...
    def calc_powers(self):
        ...
        print(len(self.events), 'ripple events')
        # self.save_ripples()
    ...
    def show_event(self, i, j=None):
        ...
        if hasattr(self, 'frame_array'):
            titletext = f'{self.prefix} t = {p1 / self.fs:.2f} s; frame = {int(self.frame_array[p1])}'
            if hasattr(self, 'accepted_events'):
                n = 0
                for key, value in self.accepted_events.items():
                    n += value
                titletext += f' {n} events of {len(self.accepted_events)} included'
    ...
    def rec_enum_ripples(self, exclude_spikes=True):
        ...
        while not self.exit:
            ...
            if bp == 's':
                ...
                self.accepted_events[i] = self.events[i].incl
                i += 1
                if i > len(self.events) - 1:
                    p = self.det_next()
                    if p is None:
    ...
    def export_ripple_times(self):
        if len(self.events) > 0:
            ...
            for i, p in enumerate(events):
                ea[i, :] = p
        pandas.DataFrame(ea,
                         columns=['RippleStart', 'RippleStop']).to_excel(self.path + self.prefix + '_ripple_times.xlsx')
    ...
    def export_matlab(self):
        ...
        scipy.io.savemat(self.path + self.prefix + '_ephysexport.mat', mat)
